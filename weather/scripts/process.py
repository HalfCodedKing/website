...
if __name__ == "__main__":
    ...
    #create map overlay
    print("creating map")
    date = datetime.strptime(p['aos'], "%Y-%m-%d %H:%M:%S.%f %Z")+timedelta(3, 0).strftime("%d %b %Y %H:%M:%S")
    subprocess.call(["/usr/local/bin/wxmap", "-T", "\"{}\"".format(sat), "-H", "/home/pi/website/weather/scripts/weather.tle", "-p", "0", "-l", "0", "-o", "\"{}\"".format(date), "{}-map.png".format(outfile)])
